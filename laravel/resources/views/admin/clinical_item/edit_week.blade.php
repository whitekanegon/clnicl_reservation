...
<!-- ページタイトルを入力 -->
@section('title', '定休日（週）、予約時間帯編集')

<!-- ページの見出しを入力 -->
@section('content_header')
<h1><span>【{{$clinical_item->name}}】</span> 定休日（週）、予約時間帯編集</h1>
@stop
@section('breadcrumb')
<ol class="breadcrumb">
...
<ul class="anchors">
	...
	<form action="edit_week" method="post" id="form_week" class="edit_week">
<div class="card box-primary" id="times">
 ...
 <div class="card-body table-responsive">
   ...
   <?php $week_id = $regular_week->week_id; ?>
			<tr>
				<td>{{$weeks->where('week_id',$week_id)->first()->name}}</td>
    <td>
					<select name="reserve_status_id[]" class="form-control inline reserve_status" data-selected="{{$regular_week->reserve_status_id}}">
      ...
      @endforeach
				...
				<td>
     <input type="hidden" name="regular_week_id[]" value="{{$regular_week->id}}">
     <input type="hidden" name="week_id[]" value="{{$week_id}}">
					<select name="reserve_times_set_id[]" class="form-control inline reserve_times_set" data-selected="{{$regular_week->reserve_times_set_id}}">
	...
	<div class="card box-primary" id="timing">
		...
		<div class="card-body">
			<div class="form-group">
				<select name="reserve_timing" data-selected="{{$clinical_item->reserve_timing}}" class="form-control inline">
     ...
     <?php
     if($clinical_item->reserve_include_holiday == 0){
      $noCount = 'checked';
      $hasCount = '';
     }else{
      $noCount = '';
     ...
     ?>
					<label>
						<input type="radio" name="reserve_include_holiday" value="0" {{$noCount}}>
						<span>数えない</span></label>
					<label>
						<input type="radio" name="reserve_include_holiday" value="1" {{$hasCount}}>
						<span>数える</span></label>
				</div>
			</div>
		</div>
	...
	<div class="card box-primary" id="range">
		...
		<div class="card-body">
			<div class="form-group">
				<select name="calendar_length" data-selected="{{$clinical_item->calendar_length}}" class="form-control inline">
...
<div class="box-fixed-btn">
	<ul>
		<li>
			<button class="btn btn-warning btn-lg btn-save" data-form="form_week">保存</button>
		</li>
	</ul>
</div>
@stop
